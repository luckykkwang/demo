...
（1）封装读取角色的信息的请求 **`src/api/settings.js`**
...
<!--
  分页
    1. 每一页显示多少条 page-size 默认10
    2. 总共有多少条  total
    3. 当前页是第几页 current-page： 1
-->
...
data() {
  const validateName = async(rule, value, callback) => {
    // 思路
    const res = await getRoleList(1, this.total)
    res.rows.some(item => item.name === value) ? callback(new Error('角色名重复')) : callback()
  }
